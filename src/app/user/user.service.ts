...
export class UserService {
    ...
    async register(dto: UserCreateDto): Promise<UserDto> {
        ...
        const existingUserEntity = await this.manager.findOneBy(UserEntity, {login: ILike(login)});

        if (existingUserEntity) {
            throw new UnprocessableEntityException('Пользователь с таким логином уже существует');
        }
    ...
    async login(dto: UserLoginRequestDto) {
        ...
        const userEntity = await this.manager.findOneBy(UserEntity, {login: ILike(login)});

        if (!userEntity) {
            throw new UnprocessableEntityException('Пользователь с таким логином не найден');
    }
